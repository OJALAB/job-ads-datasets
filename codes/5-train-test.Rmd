...
all_sources <- rbind(info[, source:="info"], kprm[, source:="kprm"], hand[, source:="hand"], cbop_test[, .(code, desc, source="cbop")])
...
```{r}
fwrite(x = test_data[, .(class=code, desc=desc)], file = "../data/test-data.tar.gz")
fwrite(x = train_data[, .(class=code, desc=desc)], file = "../data/train-data.tar.gz")
```
