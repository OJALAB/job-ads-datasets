...
```{r}
gr4 <- read_docx("../data-raw/ksiz-desc-lev4_1.docx")
gr4_tabs <- docx_extract_all_tbls(gr4, guess_header = F)
gr4_tab <- data.table::rbindlist(gr4_tabs[-c(1:11)])
...
gr4_tab[V1 == "kod",  kod:=V2]
gr4_tab[stri_detect_regex(kod, "\\d{4}",negate = T) & stri_detect_regex(V2, "\\d{4}",negate = F), kod:=V2]
