...
```{r}
all_data <- rbind(esco[, source:="esco"], thes[, source:="thes"], info[, source:="info"], kprm[, source:="kprm"], 
                  hand[str_detect(code,  "\\d{6}")][, source:="hand"], zapis[,source:="cbop"], fill = T) 

all_data[,.N, .(code, source)][, source_counts:=.N, code]|> 
  dcast(code + source_counts ~ source, value.var = "N", fill = 0)  |>
  subset(source_counts == 2)
```
